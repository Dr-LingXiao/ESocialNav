DATA_PATH=./LLaVA-Pretrain/blip_laion_cc_sbu_558k.json #pretrain annotation file path
IMAGE_PATH=./LLaVA-Pretrain #pretrain image dir
...
bash scripts/train/pretrain_siglip2_phi2_qformer.sh "$DATA_PATH" "$IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
