...
FINETUNE_DATA_PATH="snei_train_base.json"  # DPO
# FINETUNE_DATA_PATH="snei_train_base.json"   # SFT
# FINETUNE_DATA_PATH="snei_train_base_dpo.json", "snei_train_base.json"   # both
...
# bash scripts/train/pretrain_SNEI_dpo.sh "$DATA_PATH" "$IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
bash scripts/train/finetune_SNEI_dpo_1.sh "$FINETUNE_DATA_PATH" "$FINETUNE_IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$CONV_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
