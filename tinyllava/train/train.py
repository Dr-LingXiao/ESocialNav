...
def train():
    ...
    # ======== CORRECT FULL MODEL PARAM COUNT (BEFORE TRAINER) ========
    total_params = model.num_parameters()
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

    with open("/home/ling/TinyLLaVA_Factory/full_model_params.log", "w") as f:
        f.write(f"Total parameters: {total_params/1e9:.3f} B\n")
        f.write(f"Trainable parameters: {trainable_params/1e6:.3f} M\n")
