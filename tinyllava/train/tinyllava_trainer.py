...
class LLaVATrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            # # ========= ① 先判断是不是 LoRA-only 模式 =========
            # tune_llm = getattr(self.args, "tune_type_llm", "").lower()
            # tune_vision = getattr(self.args, "tune_type_vision_tower", "").lower()
            # tune_connector = getattr(self.args, "tune_type_connector", "").lower()

            # # 只训练 LLM 的 LoRA，视觉和 connector 都冻结
            #             # 只训练 LLM 的 LoRA，视觉和 connector 都冻结
            # if tune_llm == "lora" and tune_vision == "frozen" and tune_connector == "frozen":
            #     lora_params = []

            #     # 显式：按名字找到所有 LoRA 权重，并在这里重新打开 requires_grad
            ...
            #     print(f"[DEBUG][LoRA-only] num lora params = {len(lora_params)}")

            #     if len(lora_params) == 0:
            #         raise ValueError(
            #             "LoRA-only setting, but no parameters with 'lora' in their name were found.\n"
            #             "请确认：\n"
            #             "1) lora_recipe 已正确挂载 LoRA（get_peft_model 或 PeftModel.from_pretrained），\n"
            #             "2) 预训练权重目录中确实包含 LoRA adapter（adapter_config.json）。"
            #         )

            #     optimizer_cls, optimizer_kwargs = self.get_optimizer_cls_and_kwargs(self.args)

            #     # LoRA 一般不需要 weight decay，全部放在一个 group 里
